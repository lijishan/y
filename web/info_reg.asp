...
Session("GetCode")=rnd()
if b="" then
response.write "<script language=JavaScript>" & chr(13) & "alert('验证码错误，请刷新页面后重新输入！如果多次提交均显错误，请单击注册页顶部的【点这里检测你的浏览器是否支持本站提供的服务？】检测您浏览器是否支持本站服务，然后按提示操作。网页地址： http://www.job528.net/Cookie.asp ');"&"window.location.href = '');" & "history.back()" & "</script>" 
else
response.write "<script language=JavaScript>" & chr(13) & "alert('验证码错误，请重新输入！如果多次提交均显错误，请单击注册页顶部的【点这里检测你的浏览器是否支持本站提供的服务？】检测您浏览器是否支持本站服务，然后按提示操作。网页地址： http://www.job528.net/Cookie.asp');"&"window.location.href = '"&b&"'"&" </script>"
end if
Response.End
end if
end sub
