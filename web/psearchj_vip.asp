...
if Trim(request("cs"))<>"" then sql=sql+" and city like '%"&request("cs")&"%'"
...
rs.close
response.write "<br>暂时没有相关职位信息！可能有以下原因：<br>1）职位库中没有相应的职位；<br>2）你指定的条件间相互矛盾；<br><br><font color=#ff0000>建议你放宽查询条件再试！！</font>" 
else
rs.pagesize=20
...
%>
						...
						<tr>
							...
							<td width="225">
							<p align="center">&lt; <a href="psearchj_vip.asp?action=f&gzdd=<%=request("gzdd")%>&job=<%=request("job")%>&date=<%=request("date")%>&what=<%=request("what")%>">首页</a>&nbsp;&nbsp;
							<a href="psearchj_vip.asp?action=p&gzdd=<%=request("gzdd")%>&job=<%=request("job")%>&date=<%=request("date")%>&what=<%=request("what")%>">上一页</a>&nbsp;&nbsp;
							<a href="psearchj_vip.asp?action=n&gzdd=<%=request("gzdd")%>&job=<%=request("job")%>&date=<%=request("date")%>&what=<%=request("what")%>">下一页</a>&nbsp;&nbsp;
							<a href="psearchj_vip.asp?action=l&gzdd=<%=request("gzdd")%>&job=<%=request("job")%>&date=<%=request("date")%>&what=<%=request("what")%>">尾页</a> &gt;</td>
							<form method="POST" action="psearchj_vip.asp?gzdd=<%=request("gzdd")%>&job=<%=request("job")%>&date=<%=request("date")%>&what=<%=request("what")%>">
							<td width="159">							
		<p>跳转到 
