...
<%
sql="select id,name,job1,gzdd,gzcs,edu,sex,vip,rdate,zhuanyen1 from in_user where name<>'' and lock=false"
...
%>
									...
									<tr>
										<td width="95" height="30" align="center" background="imagen/web_01_05.gif" style="border-left-width: 1px; border-right-width: 1px; border-top-width: 1px; ">
										<b>姓&nbsp; 名</b></td>   
										<td width="162" height="30" align="center" background="imagen/web_01_05.gif" style="border-left-width: 1px; border-right-width: 1px; border-top-width: 1px; ">
										<b>应 聘 职 位</b></td>
										<td width="164" height="30" align="center" background="imagen/web_01_05.gif" style="border-left-width: 1px; border-right-width: 1px; border-top-width: 1px; ">
										<b>应 聘 者 专 业</b></td>
...
%>	
								<tr>
										<td style="border-left-width: 1px; border-right-width: 1px; border-top-width: 1px; " width="95" height="30" align="center" background="images/bg9.gif"><a  target =_blank href="person.asp?id=<%=rs("id")%>"><%=rs("name")%></a><% if rs("vip") then 
										response.write"&nbsp;<font color=#ff0000>[荐]</font>"
										end if %></td>
								  <td style="border-left-width: 1px; border-right-width: 1px; border-top-width: 1px; " width="162" height="30" align="center" background="images/bg9.gif"><% if isnull(rs("job1"))then
										response.write rs("zhuanyen1")+"类职位"
										else 										
										response.write rs("job1")
										end if 
										%></td>
										<td style="border-left-width: 1px; border-right-width: 1px; border-top-width: 1px; " width="164" height="30" align="center" background="images/bg9.gif"><%=rs("zhuanyen1")%></td>
...
%>
						...
						<tr>
							...
							<td width="225">
							<p align="center">&lt; <a href="psearchp_vip.asp?action=f&gzdd=<%=request("gzdd")%>&job=<%=request("job")%>&date=<%=request("date")%>&what=<%=request("what")%>">首页</a>&nbsp;&nbsp;   
							<a href="psearchp_vip.asp?action=p&gzdd=<%=request("gzdd")%>&job=<%=request("job")%>&date=<%=request("date")%>&what=<%=request("what")%>">上一页</a>&nbsp;&nbsp;   
							<a href="psearchp_vip.asp?action=n&gzdd=<%=request("gzdd")%>&job=<%=request("job")%>&date=<%=request("date")%>&what=<%=request("what")%>">下一页</a>&nbsp;&nbsp;   
							<a href="psearchp_vip.asp?action=l&gzdd=<%=request("gzdd")%>&job=<%=request("job")%>&date=<%=request("date")%>&what=<%=request("what")%>">尾页</a> &gt;</td>   
							<form method="POST" action="psearchp_vip.asp?gzdd=<%=request("gzdd")%>&job=<%=request("job")%>&date=<%=request("date")%>&what=<%=request("what")%>">
							<td width="159">							
		<p>跳转到    
